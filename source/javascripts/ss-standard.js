if (/(MSIE [7-9]\.|Opera.*Version\/(10\.[5-9]|(11|12)\.)|Chrome\/([1-9]|10)\.|Version\/[2-4][\.0-9]+ Safari\/|Version\/(4\.0\.[4-9]|4\.[1-9]|5\.0)[\.0-9]+? Mobile\/.*Safari\/|Android [1-2]\.|BlackBerry.*WebKit)/.test(navigator.userAgent) && !/(IEMobile)/.test(navigator.userAgent)) {
  ...
  if (typeof quad_legacy !== 'function') {
    ...
    var quad_legacy = function(node) {
      ...
      if (node.value) {
        node.value = quad_liga(node.value);
      } else if (node.nodeValue) {
        node.nodeValue = quad_liga(node.nodeValue);
      } else if (node.innerHTML) {
        node.innerHTML = quad_liga(node.innerHTML);
      }

    };

    var quad_getElementsByClassName = function(node, classname) {
    ...
    var quad_liga = function(that) {
      var re = new RegExp(quad_keywords.join('|').replace(/[-[\]{}()*+?.,\\^$#\s]/g, "\\$&"),"gi");
      return that.replace(re, function(v) {
        return quad_icons[v.toLowerCase()];
      });
    };

    quad_ready(function() {
      if (document.getElementsByClassName) {
        quad_legacy(document.getElementsByClassName('quadicon'));
      } else {
        quad_legacy(quad_getElementsByClassName(document.body, 'quadicon'));
      }
    });
